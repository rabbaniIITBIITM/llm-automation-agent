...
def sort_json_by_keys(input_file: str, output_file: str, keys: list):
    """
    Sort JSON data by keys in specified order and write the result to an output file.
    Args:
        input_file (str): The path to the input JSON file.
        output_file (str): The path to the output JSON file.
        keys (list): The keys to sort the JSON data by.
    ...
    normalized_data = {k.replace("\\", "/"): v for k, v in data.items()}

    # Open the output file and write sorted JSON directly
    with open(output_file_path, "w") as file:
        file.write("{\n")
        for i, (key, value) in enumerate(sorted(normalized_data.items())):
            json.dump(key, file)  # Write key
            file.write(": ")  
            json.dump(value, file)  # Write value
